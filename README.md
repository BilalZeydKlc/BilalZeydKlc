### Merhaba, Ben Bilal Zeyd Kılıç! 👋

**İstanbul Okan Üniversitesi Mobil Teknolojileri** bölümü öğrencisiyim. Mobil uygulama geliştirme iOS ve Web teknolojileri üzerine odaklanıyorum.

Teknoloji dünyasında kullanıcı deneyimini ön planda tutan projeler geliştirmeyi ve sürekli öğrenmeyi hedefliyorum.

---

### 🛠️ Yetkinlikler & İlgi Alanları

* 📱 **Mobil Geliştirme:** iOS (Swift, SwiftUI), Android (Kotlin)
* 🧠 **Programlama Temelleri:** C#, Veri Yapıları
* 🌐 **Web Geliştirme:** Frontend Teknolojileri (HTML,CSS)
* 🖨️ **Ek İlgi Alanları:** 3D Tasarım & Üretim Teknolojileri

### 🚀 Şu An Ne Yapıyorum?
- 🌱 **SwiftUI** mimarisi ve modern mobil tasarım prensipleri üzerine çalışıyorum.
- 🔭 **Programlama Temelleri:** C# ile nesne yönelimli programlama (OOP) yapılarını pekiştiriyorum.
- 🎓 Akademik projelerimle portföyümü zenginleştiriyorum.

---
